...
class JdbcGenerationJobRepository(private val dataSource: DataSource) : GenerationJobRepository {
    override suspend fun findById(id: UUID): GenerationJobRecord? = dataSource.connection.use { c ->
        c.prepareStatement("select id, project_id, type, status, progress, error, attempt from generation_jobs where id = ?").use { s ->
            s.setObject(1, id)
            s.executeQuery().use { rs ->
                if (!rs.next()) null else GenerationJobRecord(
                    ...
                    rs.getInt("progress"), rs.getString("error"), rs.getInt("attempt")
                )
            }
        }
    }
    ...
    override suspend fun save(job: GenerationJobRecord): GenerationJobRecord = dataSource.connection.use { c ->
        c.prepareStatement("insert into generation_jobs(id, project_id, type, status, progress, error, attempt) values (?, ?, ?, ?, ?, ?, ?) on conflict (id) do update set status = excluded.status, progress = excluded.progress, error = excluded.error, attempt = excluded.attempt, updated_at = current_timestamp").use { s ->
    ...
    override suspend fun updateStatus(
        ...
        val sql = if (attempt == null) {
            "update generation_jobs set status = ?, progress = ?, error = ?, updated_at = current_timestamp where id = ?"
        } else {
            "update generation_jobs set status = ?, progress = ?, error = ?, attempt = ?, updated_at = current_timestamp where id = ?"
        }
        c.prepareStatement(sql).use { s ->
            ...
            if (attempt == null) {
                ...
                s.setInt(4, attempt); s.setObject(5, id)
            }
            s.executeUpdate()
        }
    }
