...
import com.hakayat.backend.ai.StreamResponse
import com.hakayat.backend.ai.TokenUsage
...
internal fun text(json: String, key: String): String? = Regex("\\\"$key\\\"\\s*:\\s*\\\"((?:\\\\.|[^\\\"])*)\\\"").find(json)?.groupValues?.get(1)?.replace("\\\\\\\"", "\\\"")
internal fun number(json: String, key: String): Long = Regex("\\\"$key\\\"\\s*:\\s*(\\d+)").find(json)?.groupValues?.get(1)?.toLong() ?: 0
